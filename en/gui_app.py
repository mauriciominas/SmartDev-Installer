...
class SmartDevInstallerGUI(ctk.CTk):
        ...
        try:
            while not self.msg_queue.empty():
                ...
                if msg_type == "log":
                    ...
                    self.status_lbl.configure(text="Critical error during execution.", text_color=COR_FALHA)
                    self._handle_done(1)
    ...
    def _handle_done(self, returncode):
        ...
        if returncode == 0:
            ...
            self.status_lbl.configure(text="Process completed successfully!", text_color=COR_SUCESSO)
            self._append_log("\n============================================\n[COMPLETED] All steps executed.\n============================================", tag="sucesso")
        else:
            self.status_lbl.configure(text=f"Process finished with code {returncode}", text_color=COR_FALHA)
            self._append_log(f"\n[ERROR] The process returned code {returncode}.", tag="falha")
    ...
    def start_installation(self):
        ...
        self.clear_log()
        self.progress_bar.set(0)
        self.status_lbl.configure(text="Starting installation...", text_color=self._cor_status_padrao)
